...
const App: React.FC = () => {
    ...
    if (isLoading) return <div className="h-screen bg-[#080808] flex items-center justify-center text-orange-500 font-black tracking-widest animate-pulse">LOADING INTEL...</div>;
        ...
        <div className="flex h-screen overflow-hidden bg-[#0c0c0c] text-gray-200 font-inter">
            ...
            <nav className="w-64 flex-shrink-0 border-r border-white/5 bg-[#0a0a0a] flex flex-col z-30">
                <div className="p-6 border-b border-white/5">
                    ...
                    <p className="text-[8px] text-orange-500 font-bold uppercase tracking-[0.4em] mt-2 opacity-80">v1.2 Live</p>
                </div>
                ...
                <div className="flex-1 overflow-y-auto py-4 custom-scrollbar">
                    ...
                    {TRADERS.map((trader) => (
                        <button key={trader} onClick={() => setActiveTrader(trader)} className={`w-full flex items-center gap-4 px-6 py-3 border-l-4 transition-all ${activeTrader === trader ? `bg-white/5 ${TRADER_COLORS[trader]} text-white` : 'border-transparent text-gray-500'}`}>
            ...
            <main className="flex-1 flex flex-col min-w-0 relative">
                <header className="px-8 py-6 border-b border-white/5 flex justify-between items-center bg-[#0f0f0f]/90 backdrop-blur-xl z-20 sticky top-0">
                    ...
                    <div className="flex gap-4 items-center">
                        <input type="text" placeholder="Search..." className="bg-[#141414] border border-white/10 rounded-lg px-4 py-2 text-xs focus:border-orange-500/40 w-56 font-bold" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
