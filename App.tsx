...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Changed path from ./quests_updated.json to quests_updated.json for better compatibility
        const response = await fetch('quests_updated.json');
        
        if (!response.ok) {
           throw new Error(`INTELLIGENCE_LINK_FAILED: ${response.status}. Ensure data file is in project root.`);
        }
    ...
    return (
      <div className="h-screen w-full flex items-center justify-center bg-[#080808] p-10 font-inter">
        <div className="max-w-md w-full border border-red-900/40 bg-red-950/10 p-10 rounded-2xl text-center space-y-6 shadow-2xl">
           ...
           <button onClick={() => window.location.reload()} className="w-full py-4 bg-red-600 hover:bg-red-500 text-white text-[10px] font-black uppercase tracking-widest rounded-lg transition-all">Re-establish Connection</button>
        </div>
      </div>
    );
  }
    ...
    <div className="flex h-screen overflow-hidden bg-[#0c0c0c] text-gray-200 selection:bg-orange-500/30 font-inter">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-md animate-in fade-in duration-300">
          <div className="max-w-md w-full p-12 bg-[#121212] border border-red-900/30 rounded-3xl shadow-2xl text-center space-y-8">
            ...
            <div className="flex gap-4">
              <button onClick={() => setShowWipeSafeguard(false)} className="flex-1 py-4 bg-white/5 hover:bg-white/10 text-[10px] font-black uppercase tracking-widest rounded-xl">Cancel</button>
              <button onClick={handleGlobalWipe} className="flex-1 py-4 bg-red-600 hover:bg-red-500 text-white text-[10px] font-black uppercase tracking-widest rounded-xl shadow-[0_0_30px_rgba(220,38,38,0.2)]">Wipe Data</button>
            </div>
          </div>
        </div>
      )}
      ...
      <nav className="w-72 flex-shrink-0 border-r border-white/5 bg-[#0a0a0a] flex flex-col z-30">
        <div className="p-10 border-b border-white/5">
          ...
          <p className="text-[8px] text-orange-500 font-bold uppercase tracking-[0.4em] mt-3 opacity-80">v1.2 Stable</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-6 custom-scrollbar">
          ...
          >
            <span className="text-[11px] font-black uppercase tracking-[0.2em]">All Traders</span>
          </button>
          
          <div className="px-8 py-4 text-[9px] font-black text-gray-800 uppercase tracking-widest">Database</div>
        ...
        <header className="px-10 py-8 border-b border-white/5 flex flex-col xl:flex-row justify-between items-center gap-8 bg-[#0f0f0f]/90 backdrop-blur-3xl z-20 sticky top-0">
          ...
          <div className="flex flex-col sm:flex-row gap-5 w-full xl:w-auto items-center">
            ...
            <button 
              ...
              className="p-3.5 rounded-xl border border-white/5 hover:border-red-900/50 hover:bg-red-950/30 text-gray-600 hover:text-red-500 transition-all"
              title="PURGE SYSTEM"
            >
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-[#0c0c0c]/95 border-t border-white/5 backdrop-blur-2xl z-20">
          <div className="p-6 pb-4">
            <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-10">
              <ProgressBar label="Overall Quests" value={`${stats.overall.count}/${stats.overall.total}`} pct={stats.overall.pct} color="bg-orange-600" />
          ...
          <div className="w-full border-t border-white/5 py-1.5 bg-black/40 flex justify-center items-center gap-3">
             <span className="text-[8px] font-black uppercase tracking-[0.3em] text-gray-700 italic">Operational Status: Nominal</span>
        ...
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
              ...
              return (
                <button
                  ...
                  className={`aspect-square rounded-lg border transition-all duration-300 flex items-center justify-center relative group overflow-hidden ${
                    isFound ? 'bg-orange-500/10 border-orange-500/40' : 'bg-[#0f0f0f] border-white/5 opacity-20 hover:opacity-100 hover:border-white/20'
                  }`}
                  title={item}
                >
                  <img 
                    ...
                    alt=""
                    className={`w-[80%] h-[80%] object-contain ${isFound ? '' : 'grayscale'}`}
                    onError={(e) => {
                       e.currentTarget.style.display = 'none';
                       const parent = e.currentTarget.parentElement;
                       if (parent && !parent.querySelector('.fallback')) {
                         const span = document.createElement('span');
                         span.className = 'fallback text-[8px] font-black text-gray-800 text-center p-1';
          ...
          <div className="mt-10 p-8 bg-white/[0.02] border border-white/5 rounded-3xl space-y-6">
             ...
             <div className="w-full bg-black h-1.5 rounded-full overflow-hidden">
               <div className="bg-orange-500 h-full transition-all duration-1000" style={{ width: `${(foundCollectorItems.size / COLLECTOR_ITEMS.length) * 100}%` }} />
             </div>
          </div>
        </div>
      </aside>
