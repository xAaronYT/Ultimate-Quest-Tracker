...
const App: React.FC = () => {
  ...
  const filteredQuests = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      ...
      const getWeight = (d: boolean, a: boolean) => ( !d && a ? 0 : !d && !a ? 1 : 2 );
  ...
  const stats = useMemo(() => {
    ...
    return { overall: calc(allQuests), kappa: calc(allQuests.filter(q => q.kappaRequired)), lightkeeper: calc(allQuests.filter(q => q.lightkeeperRequired)) };
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#0c0c0c] text-gray-200 font-inter">
      ...
      <nav className="w-64 flex-shrink-0 border-r border-white/5 bg-[#0a0a0a] flex flex-col z-30">
        <div className="p-6 border-b border-white/5"><h1 className="text-lg font-black tracking-tighter text-white uppercase italic leading-none">Ultimate Quest Tracker</h1></div>
        <div className="flex-1 overflow-y-auto py-4 custom-scrollbar">
          ...
          {TRADERS.map((t) => (
            <button key={t} onClick={() => setActiveTrader(t)} className={`w-full flex items-center gap-4 px-6 py-3 border-l-4 ${activeTrader === t ? 'bg-white/5 border-orange-500 text-white' : 'border-transparent text-gray-500 hover:text-orange-400'}`}>
              <div className="w-7 h-7 rounded bg-black/40 border border-white/5 overflow-hidden"><img src={`/assets/${normalizeAssetName(t)}.png`} className="w-full h-full object-cover" /></div>
        ...
        <div className="p-4 bg-black/60 border-t border-white/5 space-y-2">
          ...
          <a href="https://cash.app/$xajcinc" target="_blank" className="flex flex-col items-center p-3 rounded-xl bg-orange-500/10 border border-orange-500/20 hover:border-orange-500/50 transition-all group">
      ...
      <main className="flex-1 flex flex-col relative">
        <header className="px-8 py-6 border-b border-white/5 flex justify-between items-center bg-[#0f0f0f] z-20 sticky top-0">
          ...
          <div className="flex gap-4 items-center">
            <input type="text" placeholder="Search..." className="bg-[#141414] border border-white/10 rounded-lg px-4 py-2 text-xs w-56 font-bold outline-none focus:border-orange-500/50" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
            <div className="flex bg-black p-1 rounded-lg border border-white/5">
              {(['Active', 'Kappa', 'Lightkeeper', 'Show All'] as FilterMode[]).map((m) => (
                <button key={m} onClick={() => setFilterMode(m)} className={`px-4 py-1.5 rounded text-[9px] font-black uppercase tracking-widest ${filterMode === m ? 'bg-orange-500 text-black' : 'text-gray-600'}`}>{m}</button>
              ))}
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 pb-[160px] custom-scrollbar">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {filteredQuests.map(q => <QuestCard key={q.id} quest={q} isCompleted={completedQuestIds.has(q.id)} isAvailable={checkAvailability(q)} onToggle={toggleQuest} />)}
          </div>
        </div>
      ...
      <aside className="hidden 2xl:flex w-80 flex-shrink-0 border-l border-white/5 bg-[#0a0a0a] flex flex-col z-30">
        <div className="p-6 border-b border-white/5"><h3 className="text-[10px] font-black text-orange-500 uppercase tracking-[0.3em] italic">The Stash</h3></div>
...
const StashItem: React.FC<{ item: string, isFound: boolean, onToggle: (item: string) => void }> = ({ item, isFound, onToggle }) => {
  ...
  return (
    <button onClick={() => onToggle(item)} className={`relative aspect-square rounded-md border transition-all duration-200 flex items-center justify-center overflow-hidden ${isFound ? 'bg-orange-500/20 border-orange-400 shadow-[0_0_10px_rgba(251,146,60,0.2)]' : 'bg-zinc-900/50 border-white/10 opacity-60 hover:opacity-100'}`}>
      {!imgErr ? <img src={`/assets/items/${normalizeAssetName(item)}.png`} className={`w-[85%] h-[85%] object-contain ${!isFound ? 'brightness-75 contrast-125' : 'brightness-110'}`} onError={() => setImgErr(true)} /> : <span className="text-[7px] font-black text-gray-500 uppercase p-1">{item}</span>}
